...
class ExprTyper(namer: Namer):
  ...
  : TypeTree =
    ...
    if isOperatorExpr then
      ...
      namer.transformType(word, checkKindIfRef = true, allowPackType = allowPackType)
  ...
  : TypeTree =
          ...
          case Ast.Ident(name) if Naming.isOperator(name) =>
            val typed =
              tpt.getKeyOrUpdate(Namer.TypedTypeTree):
                namer.transformType(tpt, checkKindIfRef = true, allowPackType = allowPackType && count <= 1)
    ...
    namer.transformType(typeTrees.last, checkKindIfRef = true, allowPackType = allowPackType)
