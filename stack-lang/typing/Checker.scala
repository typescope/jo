...
object Checker:
  ...
  /** Validate a view specification
    *
    * Checks that the adapter (if present) is a valid function for converting
    * the base type to the view type.
    *
    * @return Some(viewSpec) if valid, None if validation fails
    */
  def checkViewSpec(viewSpec: ViewSpec, baseType: Type, astViewSpec: Ast.ViewSpec)
      (using defn: Definitions, rp: Reporter, so: Source)
  : Option[ViewSpec] =
    // Check: view type must be a class or interface (not a type alias)
    viewSpec.viewType match
      case StaticRef(sym) if sym.isType && sym.isAlias =>
        Reporter.error(s"View type cannot be a type alias, found = ${viewSpec.viewType.show}", astViewSpec.pos)
        return None
      case _ =>
        if !viewSpec.viewType.approx.isClassInfo then
          Reporter.error(s"View type must be a class or interface, found = ${viewSpec.viewType.show}", astViewSpec.pos)
          return None
    ...
    viewSpec.adapter match
      ...
      case Some(adapterSym) =>
        if !adapterSym.is(Flags.Fun) then
          Reporter.error("View adapter must be a function, found = " + adapterSym, astViewSpec.pos)
          None
        else
          ...
          if procType.params.size != 1 then
            Reporter.error(s"View adapter must take exactly one parameter, found ${procType.params.size} parameters", astViewSpec.pos)
            None

          // Check: must have no auto parameters
          else if procType.autos.nonEmpty then
            Reporter.error("View adapter cannot have auto parameters", astViewSpec.pos)
            None

          // Check: must have no type parameters
          else if procType.tparams.nonEmpty then
            Reporter.error("View adapter cannot have type parameters", astViewSpec.pos)
            None
          ...
          // Check: parameter type must conform to base type
            ...
            if !Subtyping.conforms(baseType, adapterParamType) then
              Reporter.error(s"Base type ${baseType.show} does not conform to adapter parameter type ${adapterParamType.show}", astViewSpec.pos)
              None

            // Check: return type must conform to view type
            else if !Subtyping.conforms(procType.resultType, viewSpec.viewType) then
              Reporter.error(s"Adapter return type ${procType.resultType.show} does not conform to view type ${viewSpec.viewType.show}", astViewSpec.pos)
              None
