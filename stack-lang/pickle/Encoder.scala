...
object Encoder:
  ...
  /*
  private trait Encoder[T]:
    def encode(item: T)(using Definitions, State, WriteBuffer): Unit

  private given Encoder[Word] =
    new Encoder[Word]:
      def encode(item: Word)(using Definitions, State, WriteBuffer) =
        encodeWord(item)

  private given Encoder[Pattern] =
    new Encoder[Pattern]:
      def encode(item: Pattern)(using Definitions, State, WriteBuffer) =
        encodePattern(item)

  private given Encoder[Type] =
    new Encoder[Type]:
      def encode(item: Type)(using Definitions, State, WriteBuffer) =
        encodeType(item)

  private given Encoder[TypeTree] =
    new Encoder[TypeTree]:
      def encode(item: TypeTree)(using Definitions, State, WriteBuffer) =
        encodeTypeTree(item)

  private given Encoder[Flags] =
    new Encoder[Flags]:
      def encode(value: Flags)(using Definitions, State, WriteBuffer) =
        encodeFlags(value)

  private given Encoder[Boolean] =
    new Encoder[Boolean]:
      def encode(value: Boolean)(using Definitions, State, WriteBuffer) =
        encodeBool(value)

  private given Encoder[Byte] =
    new Encoder[Byte]:
      def encode(value: Byte)(using Definitions, State, WriteBuffer) =
        encodeByte(value)

  private given Encoder[Int] =
    new Encoder[Int]:
      def encode(value: Int)(using Definitions, State, WriteBuffer) =
        encodeInt(value)

  private given Encoder[String] =
    new Encoder[String]:
      def encode(value: String)(using Definitions, State, WriteBuffer) =
        encodeString(value)

  private given Encoder[Source] =
    new Encoder[Source]:
      def encode(value: Source)(using Definitions, State, WriteBuffer) =
        encodeSource(value)

  private def <<<[T](item: T)(using defn: Definitions, state: State, buf: WriteBuffer, enc: Encoder[T]) =
    enc.encode(item)

  /** The allocation of Encoder[Iterable[T]] can be eliminated after inlining */
  private given iterableEncoder[T](using enc: Encoder[T]): Encoder[Iterable[T]] =
    new Encoder[Iterable[T]]:
      def encode(items: Iterable[T])(using defn: Definitions, state: State, buf: WriteBuffer) =
        repeated(items) { item => enc.encode(item) }
  */
