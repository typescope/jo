...
/** Byte-level mutators. These are deliberately language-oblivious — they run
  * on raw bytes, so they exercise the scanner and the parser's recovery paths
  * on inputs that range from "slightly corrupted seed" to "line noise".
  *
  * Token-level and grammar-directed generators live in later modules; this
  * one is the cheapest and the first to ship.
  */
object Mutator:
  ...
  private val Punctuation: Array[String] = Array("(", ")", "[", "]", "{", "}", ",", ";")
