...
extends Phase[Symbol]:
  ...
  override def transformWith(word: With)(using ctx: Context): Word =
    ...
    val hasXSyms = args.map: arg =>
      ...
      val hasXSym = Symbol.createSymbol("has_" + paramName, BoolType, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = arg.rhs.pos)
    ...
    val oldValueSyms = args.zip(argValueSyms).map: (arg, argValueSym) =>
      ...
      val oldValueSym = Symbol.createSymbol("old_" + paramName, arg.rhs.tpe, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = arg.rhs.pos)
      stats += Assign(Ident(oldValueSym)(arg.ident.span), setParamCall.encodedAs(arg.rhs.tpe))
      oldValueSym

    // 4. val res = expr only if expr is not void
    val resSym = Symbol.createSymbol("res", expr.tpe, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = expr.pos)
