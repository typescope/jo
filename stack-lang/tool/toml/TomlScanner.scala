...
class TomlScanner(input: String):
  ...
  private def next(): ScannedToken =
    ...
    ch match
      ...
      case '[' => pos += 1; ScannedToken(Token.TLBracket, startLine)
      case ']' => pos += 1; ScannedToken(Token.TRBracket, startLine)
      case '{' => pos += 1; ScannedToken(Token.TLBrace, startLine)
      case '}' => pos += 1; ScannedToken(Token.TRBrace, startLine)
      case '=' => pos += 1; ScannedToken(Token.TEquals, startLine)
      case ',' => pos += 1; ScannedToken(Token.TComma, startLine)
      case '.' => pos += 1; ScannedToken(Token.TDot, startLine)
