...
object DependencyResolver:
  ...
  def resolve(spec: BuildSpec)(using provider: PackageProvider): Result[List[ResolvedPackage]] =
    val constraints = mutable.LinkedHashMap.empty[String, List[String]]
    ...
    spec.main.dependencies.foreach:
      case (name, DepSpec(DepSource.Registry(constraint), _)) =>
        queue.enqueue((name, constraint))

      case _ =>

    while queue.nonEmpty do
      val (name, constraint) = queue.dequeue()
      val currentConstraints = constraints.getOrElse(name, Nil)
      if !currentConstraints.contains(constraint) then
        constraints(name) = constraint :: currentConstraints
          ...
          provider.path(name, version).map: path =>
            ...
            if changed then
              meta.dependencies.foreach: (depName, depConstraint) =>
                queue.enqueue((depName, depConstraint))
      match
        case Result.Ok(_) =>
  ...
  private def selectVersion(name: String, constraints: List[String])(using provider: PackageProvider): Result[Version] =
    provider.versions(name).flatMap: versions =>
      val sorted = versions.sorted.reverse
      sorted.find(v => constraints.forall(c => Version.satisfiesConstraint(v, c))) match
        case Some(v) => Result.Ok(v)
        case None =>
          Result.Err(
            s"conflicting requirements for $name: ${constraints.distinct.sorted.mkString(", ")}"
          )
