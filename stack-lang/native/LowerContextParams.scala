...
class LowerContextParams(runtime: NativeRuntime)(using defn: Definitions) extends phases.Phase[Symbol]:
  ...
  override def transformWith(word: With)(using ctx: Context): Word =
    ...
    val argValueSyms = args.map: arg =>
      ...
      val argValueSym = Symbol.createSymbol("arg_" + paramName, arg.rhs.tpe, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = arg.rhs.pos)
    ...
    val restorePairSyms = args.zip(argValueSyms).map: (arg, argValueSym) =>
      ...
      val hashIndexSym = Symbol.createSymbol("hash_index_" + paramName, IntType, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = arg.rhs.pos)
      ...
      val oldValueSym = Symbol.createSymbol("old_value_" + paramName, arg.rhs.tpe, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = arg.rhs.pos)
    ...
    val resSym = Symbol.createSymbol("res", expr.tpe, Flags.Synthetic, owner = ctx, visibility = Visibility.Scope, pos = null)
    ...
    paramRefs.zip(restorePairSyms).foreach:
      case (paramRef, (hashIndexSym, oldValueSym)) =>
        ...
        val restoreParamCall = restoreParam.appliedTo(index, value).dropValue
