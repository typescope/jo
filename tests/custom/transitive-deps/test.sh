...
bin/jo build-lib "$DIR/core.stk" -d "$BUILD_CORE"

# Build validation library (depends on core)
echo "  - Building validation library"
bin/jo build-lib "$DIR/validation.stk" -lib "$BUILD_CORE" -d "$BUILD_VALIDATION"

# Build processor library (depends on core and validation)
echo "  - Building processor library"
bin/jo build-lib "$DIR/processor.stk" -lib "$BUILD_CORE:$BUILD_VALIDATION" -d "$BUILD_PROCESSOR"
