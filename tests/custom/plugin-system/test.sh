...
bin/jo build-lib "$DIR/plugin.stk" -lib "$BUILD/framework" -d "$BUILD/plugin"
...
bin/jo run "$DIR/app.stk" -lib "$BUILD/framework:$BUILD/plugin" $LINK_FLAGS > "$DIR/actual.out" 2>&1
...
bin/jo build -reg "$DIR/app.stk" -lib "$BUILD/framework:$BUILD/plugin" $LINK_FLAGS -o "$DIR/app.run"
...
bin/jo build -stack "$DIR/app.stk" -lib "$BUILD/framework:$BUILD/plugin" $LINK_FLAGS -o "$DIR/app.run"
"$DIR/app.run" > "$DIR/actual.out" 2>&1
diff "$DIR/actual.out" "$DIR/expect.check" || {
...
bin/jo build -js "$DIR/app.stk" -lib "$BUILD/framework:$BUILD/plugin" $LINK_FLAGS -o "$DIR/app.js"
