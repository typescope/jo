...
date: 2026-06-08
author: The Jo Team
description: We benchmark 8 frontier models on Jo, a newly designed programming language, compare efficiency and accuracy between models and agent strategies, analyze common failure patterns across model.
---
...
Since [HumanEval](https://arxiv.org/abs/2107.03374) started code generating evaluation on large language models(LLMs), many benchmarks have been proposed to evaluate LLMs from different coding task scenarios. [MBPP](https://arxiv.org/pdf/2108.07732) evaluates how well LLMs can generate short Python programs from natural language descriptions. [SWE-bench](https://arxiv.org/pdf/2310.06770) assesses the ability of LLMs to tackle real-world software issues sourced from GitHub. These benchmarks all target the `python` language. Some have scores approching to 100% ([human eval](https://llm-stats.com/benchmarks/humaneval), [mbpp](https://llm-stats.com/benchmarks/mbpp)).

[EsoLang-Bench](https://esolang-bench.vercel.app/) is designed specificially to test LLMs on esoteric programming languages. In contrast, all the frontier models perform poorly on simple algorithmic problems. The best accuracy is only 3.8%. The languages `EsoLang-Bench` tested on are not real programming languages, but intentionally created as a proof of concept. It will be interesting to see how LLMs perform on a genuine new program language, what kind of issues the models will encounter and how we can make them achieve better efficiency and accuracy.
...
In this research, we evaluate coding capability of LLMs on  `Jo`. We compare efficiencies between different models by two distinct agent strategies. We also analyze common error patterns from failed problems and verify impact of improved language syntax prompt on LLMs' coding efficiency.

## 2. Benchmark Setup

We use three(easy, medium, and hard) algorithmic problem sets from [EsoLang-Bench](https://esolang-bench.vercel.app/) for testing. Two strategies(zero-shot and self-refine) are tested for each model on each problem set. `Zero-shot` strategy provides models with a simple Jo syntax doc as initial prompt. Each model can only attempt a solution once per problem — pass or fail, no retry. `Self-refine` strategy extends zero-shot by allowing the model to iterate on incorrect solutions using the compiler or runtime error output. The maximum number of attempts per problem is set to 5.
...
The benchmark source code, testing data and setup scripts and guide documents are included in the [benchmark repo](https://github.com/typescope/agent-bench). The testing source code is easily extensible to add a new agent, model or language for future research.

## 3. Overall Results

The pass rate is calculated from dividing the number of correctly solved problems by total problems in a set(20). The rate drops smoothly from top to bottom in ranking order. Haiku is an outlier, scoring only around 20% on zero-shot. We will analyse the primary failures on Haiku in later sections.

![pass rate heatmap](assets/heatmap_pass_rate.png)

Strategy is a major factor impacting pass rate. Under one-shot strategy, mid-tier models like GPT-mini, GPT-nano, and Gemini-flash score only ~50% on the hard problem set, self-refine pushes that to above 85%.
...
Latency are roughly on par except GPT with self-refine strategy. Self-refine has average higher latency due to multiple attempts. Qwen and DeepSeek are clear outliers. The abnormality is likely caused by rate limit thresholds on their serving platforms in Asia.

![API latency heatmap](assets/elapsed_heatmap.png)

## 4. The Self-Refine Effect

**Self-refine resolves most of the failures encountered under zero-shot** This effect is univeral across all models, although there's noticable difference between models. E.g, Deepseek only resolves about 50% of the failures. 
...
| sonnet-4-6 | 5 | 5 | 0 |
| deepseek | 20 | 11 | 9 |
| gemini-3-flash-preview | 18 | 15 | 3 |
| gpt-5.4 | 11 | 11 | 0 |
| gpt-5.4-mini | 19 | 16 | 3 |
| gpt-5.4-nano | 23 | 20 | 3 |
| qwen | 6 | 6 | 0 |
...
| Runtime Eror | 1 | 2 | 3 | 3 | 4 | 3 | 3 | 1 |

Key observations:
- **Pattern E dominates haiku** — 39 of 48 haiku failures include a Pattern E error; 37 of those trace to a single missing `import jo.IO.*`, explaining why self-refine recovers haiku to ~90%.
- **Pattern A (missing/wrong API methods) is the most widespread error class** — DeepSeek leads at 11, GPT-mini at 9; absent string methods (`.drop`, `.reverse`, `.toUpperCase`) are the single most recurring source across all models.
...
- **Pattern D (type system assumptions) is Gemini's signature** — 6 problems, concentrated in null checks, union leading pipe, and void-return mismatches; Gemini transfers more type-system assumptions from other languages than any other model.
...
Based on the failure analysis, we revised the initial Jo prompt syntax document. Using the new document, we tested again on 5 middle or lower pass rate models.
...
37 of haiku's 48 zero-shot failures were caused by a missing import, and the same issue appears in gpt-nano.
...
All five tested models improved efficiency with the new revised prompt document. The chart below compares zero-shot pass rate before and after the doc revision across all difficulty levels.
...
**Hard problems benefited most.** Every model improved most sharply on hard problems: gpt-mini +40pp, gpt-nano +45pp, deepseek +35pp, gemini +45pp. The revised doc's additions on collection immutability, typed empty init, and null/no-op semantics directly address the Map/Set-heavy logic that hard problems require.

**Medium problems is the remaining weak spot.** Gemini regressed slightly (80%→75%); haiku made the smallest gain (25%→65%). Medium problems continue to surface List equality, tuple destructuring, and pattern matching syntax issues — gaps the revised doc did not address.

**Deepseek leads all models at 95%.** The typed init and immutability callouts resolved its Map/Set issues, and it avoids most remaining failure patterns.


### Error Delta

The table compares error pattern counts for the five retested models before and after the doc revision. OLD counts use the multi-pattern classification from section 5 (a single failure may carry multiple tags); NEW counts use primary-pattern classification from the revised doc runs.

| Pattern | OLD | NEW | Change |
|---|---:|---:|---:|
| A: Missing/wrong API | 32 | 24 | −8 |
...
Frontier models can effectively solve coding problems on newly designed language `Jo` with simple syntax prompt, but the efficiency and accuracy vary across models and agent testing strategies.
...
**Failures expose training provenance as much as model capability.** Models transfer collection and string idioms from Scala, Java, or Python. Some fail heavily on type-system assumptions; others spread failures broadly across syntax, API, and logic, indicating liberal interpolation from memory.

**Documentation quality has asymmetric leverage.** The revised prompt yielded 35–45 pp gains on hard problems across all five retested models, and eliminated entire categories of collection-related failures. However, failures persisted in absent stdlib knowledge and deeplyingrained type-system assumptions from other languages.
...
- Non-existent collection types: `Array.fill(n)(true)`, `Queue`, `Stack`, `StringBuilder`
    - Models apply Scala/Java mental model when no Jo equivalent is documented
