...
!!! note
    `meta.toml` and the build spec (`jo.toml`) serve different purposes: the build spec is instructions for *producing* an artifact; `meta.toml` describes the *produced* artifact for consumers. `jo build-release` derives `meta.toml` from the build spec — the reverse is not possible.
