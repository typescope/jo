...
Each `.joy` archive contains a `meta.toml` which contains metadata about the package. The file is generated by the compiler (`jo build-release`).

!!! note
    Library metadata (`meta.toml`) and build spec (`jo.toml`) serve different purposes: the build spec is instructions for *producing* an artifact; `meta.toml` describes the *produced* artifact for consumers. `jo build-release` derives `meta.toml` from the build spec.
