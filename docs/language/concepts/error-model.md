...
**Recoverable errors** — item not found, parse failure, network timeout — are expected conditions that callers intend to handle gracefully. They are modeled as values and handled explicitly at the call site.
...
Given `e rescue pat => block` where `e` has the type `Ta | Tb` (a **two-branch** union type after dealiasing):
...
`Result` and `Option` in standard library plays in the protocol:
...
Silently discarding a non-Unit return value is almost always a bug --
in particular, when the return value is of a union type to represent possible errors.
The compiler warns when a value is used in statement position without binding its result:
...
Meanwhile, `rescue` may be used for error correction in addition to error propagation. It also easily handles error type conversion in error propagation.
