...
One design alternative is to use the lower/upper-case of the first letter as
...
At the container level (namespace or section level), names are visible according to their visibility modifier: public (default), `private` (immediate container only), or `private[N]` (container `N` and its nested containers). Private symbols cannot appear in more public definitions.

See [Visibility and Coherence](visibility-coherence.md) for the full rules.

### Local Scoping

Names introduced by definitions (`val`, `var`, `def`, etc.) inside a block are only visible for **later phrases** within that block.

Local names are visible only from their definition point onward, scoped to their enclosing block. Inner blocks can shadow outer names.

```jo
def example(): Int =
  val x = 10
  val y = x + 5       // x visible here

  if y > 10 then
    val z = y * 2     // z scoped to this block
    z
  else
    y                 // z NOT visible here
...
Closures capture names from enclosing scopes:

```jo
def outer(): Int =
  val x = 10
  def inner(): Int = x + 5   // captures x from outer
  inner()
```
